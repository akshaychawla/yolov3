...
from utils.distill_utils import Distillation
...
def train(hyp):
    ...
    teacher = Darknet(opt.cfg_teacher).to(device)
    teacher.load_state_dict(torch.load(opt.weights_teacher, map_location=device)["model"], strict=True)
    teacher.eval()
    ...
    if mixed_precision:
        ...
        teacher, _       = amp.initialize(teacher, []     , opt_level='O1', verbosity=0)
        teacher.eval()
    ...
    if device.type != 'cpu' and torch.cuda.device_count() > 1 and torch.distributed.is_available():
        ...
        teacher.eval()
        teacher.yolo_layers = teacher.module.yolo_layers
    ...
    # Distillation
    distill_criterion = Distillation(method=opt.distill_method)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            pred = model(imgs)
            with torch.no_grad():
                teacher_inf, teacher_pred  = teacher(imgs)
            ...
            # Backward
            loss *= opt.alpha_yolo
            loss += distill_loss * opt.alpha_distill
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg-teacher', type=str, required=True, help='*.cfg path of teacher')
    ...
    parser.add_argument('--weights-teacher', type=str, required=True, help='teacher weights path')
